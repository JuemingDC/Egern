...
async function fetchApi(ctx, capture, path) {
...
async function runTask(ctx, capture) {
  const msgs = [];

  try {
    const balance = await fetchApi(ctx, capture, 'queryBalanceAndBonus');

    if (balance.retcode === 0) {
      msgs.push(`💰 余额：${balance.result?.balance ?? '?'} Coins`);
    } else {
      msgs.push(`⚠️ 查询：${balance.retmsg || '失败'}`);
    }
  } catch (e) {
    msgs.push(`❌ 查询：${e.message || String(e)}`);
  }

  try {
    const checkIn = await fetchApi(ctx, capture, 'checkIn');
    ...
    msgs.push(`❌ 签到：${e.message || String(e)}`);
  }
  ...
  for (let i = 1; i <= MAX_VIDEO; i++) {
    ...
    try {
      const video = await fetchApi(ctx, capture, 'videoBonus');
      ...
      msgs.push(`❌ 视频${i}：${e.message || String(e)}`);
      break;
    }
  }

  try {
    const latest = await fetchApi(ctx, capture, 'queryBalanceAndBonus');

    if (latest.retcode === 0) {
      msgs.push(`💰 最新余额：${latest.result?.balance ?? '?'} Coins`);
    }
  } catch (e) {
    msgs.push(`⚠️ 最新余额查询失败：${e.message || String(e)}`);
  }
...
export default async function(ctx) {
  ...
  if (!raw) {
    notify(ctx, '❌ 请先获取 PingMe 签到参数', '启用 MitM 后打开 PingMe，触发一次 queryBalanceAndBonus 请求。');
    return;
  }
  ...
  try {
    ...
    notify(ctx, '❌ PingMe 签到参数损坏', '请重新打开 PingMe 触发抓参。');
    return;
  }

  if (!capture || !capture.paramsRaw || !capture.headers) {
    notify(ctx, '❌ PingMe 签到参数不完整', '请重新打开 PingMe 触发抓参。');
    return;
  }

  try {
    const result = await runTask(ctx, capture);
    notify(ctx, '🎉 任务完成', result.slice(0, 4000));
  } catch (e) {
    notify(ctx, '❌ 任务失败', e.message || String(e));
  }
}
