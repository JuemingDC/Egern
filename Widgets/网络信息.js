/**
 * 📌 桌面小组件: 🛡️ 网络诊断雷达 (全栈解锁 Pro 版 - 金色 / 奶油黄 / 暗紫深蓝配色版)
 * 🎨 Egern Widget DSL 配色优化版
 * 
 * ===== chance 修改 =====
 * 仅修改配色逻辑：
 * - 保留早晚轮换背景
...
export default async function(ctx) {
  ...
  const C = {
    // ===== chance 修改：背景色改为金色 / 奶油黄 + 暗紫 / 深蓝 =====
    bg: isNightTheme
      ? { light: '#211A3D', dark: '#0B132B' }   // 夜晚：暗紫 / 深蓝
      : { light: '#FFF4CF', dark: '#6A4B16' },  // 白天：奶油黄 / 深金棕
    // ===== chance 修改结束 =====
    ...
    cpu: isNightTheme
      ? { light: '#8FA7FF', dark: '#89B4FA' }   // 左侧本地列：夜晚蓝紫
      : { light: '#B7791F', dark: '#F6C85F' },  // 左侧本地列：金色

    mem: isNightTheme
      ? { light: '#C4A7E7', dark: '#CBA6F7' }   // 右侧代理列：夜晚紫
      : { light: '#D69E2E', dark: '#FFE08A' },  // 右侧代理列：奶油金

    disk: isNightTheme
      ? { light: '#F6C177', dark: '#FAB387' }   // 中危 / 机房
      : { light: '#B45309', dark: '#FFB454' },

    netRx: isNightTheme
      ? { light: '#7FD6A4', dark: '#A6E3A1' }   // 纯净 / 原生住宅
  ...
  return {
    type: 'widget', 
    padding: 14,
    backgroundColor: C.bg, // ===== chance 修改：背景随早晚自动轮换 =====
    children: [
      // 顶部 Header
